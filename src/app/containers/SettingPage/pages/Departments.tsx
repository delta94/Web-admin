...
import { ModalAddDepartment } from '../components/Department/ModalAddDepartment';
import { ModalUpdateDepartment } from '../components/Department/ModalUpdateDepartment';
...
export function Department() {
  ...
  useEffect(() => {
    ...
    dispatch(actions.getAllManageType(SettingManageType.Department));
    dispatch(actions.getManageType(SettingManageType.Department));
  }, []);

  useEffect(() => {
    if (!_.isEmpty(groupServiceList)) {
      setGroupServices(groupServiceList);
    }
  }, [groupServiceList]);
  ...
  const setOpenUpdateTitleModal = department => {
    setOpenUpdateTitle(true);
    dispatch(actions.setTempManageType(department));
  };
  ...
  const renderPagination = () => {
    if (!_.isEmpty(groupServiceSlice) && !_.isEmpty(groupServiceList)) {
      return (
        <TablePagination
          ...
          count={
            keySearch !== ''
              ? groupServiceSlice.length
              : groupServiceList.length
          }
  ...
  return (
    <div className="row">
      {openAddTitle ? (
        <ModalAddDepartment open={true} onClose={closeDialog} />
      ) : null}
      {openUpdateTitle ? (
        <ModalUpdateDepartment open={true} onClose={closeDialog} />
      ) : null}
      <ModalConfirm
